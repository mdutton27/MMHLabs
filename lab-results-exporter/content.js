...
(function() {
  ...
  // Parse tabular format: "TESTNAME   VALUE   UNIT   (RANGE)"
  ...
  function parsePanelBody(body, groupName) {
    ...
    // Handle tabular (Ref. Range) format - e.g. SHBG
    if (hasRefRange) {
    ...
    // Always parse B>SPAN pairs (works for most result types)
    ...
    for (const b of bElements) {
      ...
      // Skip if no value, or if already captured via tabular parsing
      if (!valueText) continue;
      const testName = label.replace(/:$/, '').trim();
      // Avoid duplicating tabular items
